...
<template>
  <div>
    ...
    <p class="mt-0.5 text-[13px] text-grau-50">{{ waitingFor.items.length }} items delegated</p>

    <div v-if="waitingFor.loading" class="mt-4 space-y-3">
      <Skeleton v-for="i in 4" :key="i" class="h-16 w-full rounded-md" />
    </div>

    <div v-else class="mt-4 divide-y divide-grau-5">
      <div v-for="item in waitingFor.items" :key="item.id" class="py-3 px-1">
        <div class="text-sm text-schwarz">{{ item.title }}</div>
        <div v-if="item.projectName" class="mt-0.5 text-xs text-grau-50">{{ item.projectName }}</div>
        <div class="mt-1.5 flex gap-3 text-xs">
          <span v-if="item.delegatedTo" class="text-blau-50">👤 {{ item.delegatedTo }}</span>
          <span :class="daysSince(item.createdDate) >= 7 ? 'text-rot-50' : 'text-grau-20'">
