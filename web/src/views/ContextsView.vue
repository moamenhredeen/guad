...
<template>
  <div>
    ...
    <div v-if="showAdd" class="mt-3 flex gap-2">
      ...
      <Button size="sm" class="bg-schwarz text-white" @click="addContext">Save</Button>
    </div>
    ...
    <div class="mt-4 divide-y divide-grau-5">
      <div v-for="ctx in contexts.items" :key="ctx.id" class="flex items-center justify-between py-3 px-1">
        <div>
          ...
          <div v-if="ctx.description" class="mt-0.5 text-xs text-grau-20">{{ ctx.description }}</div>
        </div>
        <button class="text-grau-20 hover:text-rot-80" @click="contexts.remove(ctx.id)">
          <Trash2 class="size-4" />
        </button>
      </div>
    </div>
